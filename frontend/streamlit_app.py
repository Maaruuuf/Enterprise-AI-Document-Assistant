...
Deployment note:
-------------------------------------------
Instead of calling the API over HTTP, this version imports
...
# Show suggestion buttons only for a fresh, empty thread — they disappear
# once the conversation has started, similar to ChatGPT's initial prompts.
if not active_thread["messages"] and not user_question:
